...
// "Recently restarted" detection. Workers normally come online within seconds of the main
// process; one that came online much later was respawned after a crash (uncaught exception).
// A worker is flagged only when it started well after main (RESTART_GRACE, beyond any normal
// staggered-boot spread) AND that restart is still recent (RECENT_WINDOW), so a long-since
// recovered worker fades back to healthy while a flapping one stays flagged.
const WORKER_RESTART_GRACE = 3 * 60 * 1000; // 3 min after main = a restart, not normal boot
const WORKER_RESTART_RECENT_WINDOW = 30 * 60 * 1000; // keep flagging for 30 min after the restart
...
function init(args) {
    ...
    server.route({
        ...
        async handler(request, h) {
            ...
            // Reference point for "recently restarted" detection: the main thread's online
            // time is the process start, so any worker that came online well after it was
            // respawned after boot rather than started with the fleet.
            let now = Date.now();
            let mainThread = threads.find(threadInfo => threadInfo.isMain || threadInfo.type === 'main');
            let mainOnline = mainThread && typeof mainThread.online === 'number' ? mainThread.online : null;
            ...
            let processedThreads = threads.map(threadInfo => {
                ...
                // A worker that came online well after the main process, recently, was most
                // likely respawned after an uncaught exception. Surface it as a warning even
                // when its health checks are otherwise passing; a real error/warning health
                // status is more specific and takes precedence.
                if (
                    !threadInfo.isMain &&
                    mainOnline !== null &&
                    typeof threadInfo.online === 'number' &&
                    threadInfo.online - mainOnline > WORKER_RESTART_GRACE &&
                    now - threadInfo.online < WORKER_RESTART_RECENT_WINDOW &&
                    STATUS_RANK[threadInfo.statusVariant] < STATUS_RANK.warning
                ) {
                    threadInfo.statusVariant = 'warning';
                    threadInfo.statusLabel = 'Recently restarted';
                    threadInfo.recentlyRestarted = true;
                }
                ...
                // Email (sync) workers own account assignment, but the count field is absent
                // when a worker currently syncs none - show an explicit 0 instead of a blank.
                if (threadInfo.type === 'imap' && typeof threadInfo.accounts !== 'number') {
                    threadInfo.accounts = 0;
                }
            ...
            let groups = orderedTypes.map(type => {
                ...
                return {
                    ...
                    threads: list,
                    // Only the sync (email) workers are assigned accounts, so the Accounts
                    // column is shown only for that pool.
                    showAccounts: type === 'imap',
                    hasExpected,
